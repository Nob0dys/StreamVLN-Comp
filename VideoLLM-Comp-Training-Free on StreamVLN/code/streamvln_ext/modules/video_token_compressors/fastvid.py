...
from streamvln_ext.modules.video_token_compressors.visionzip import _token_scores
...
def _compress_frame_sequence(
    ...
    frame_retain_num = max(1, min(tokens_per_frame, int(round(tokens_per_frame * float(retention_ratio)))))
    frame_salient_num = max(1, min(frame_retain_num, frame_retain_num - int(round(frame_retain_num * float(stprune_d)))))
...
def _compress_image_sequence(
    ...
    compressed_frames, frame_sizes, meta = _compress_frame_sequence(
        frame_sequence,
        retention_ratio=ext_flags.fastvid_retention_ratio,
        dyseg_c=ext_flags.fastvid_dyseg_c,
        dyseg_tau=ext_flags.fastvid_dyseg_tau,
        stprune_d=ext_flags.fastvid_stprune_d,
        dtm_p=ext_flags.fastvid_dtm_p,
        dtm_beta=ext_flags.fastvid_dtm_beta,
        score_type=str(ext_flags.fastvid_score_type).strip().lower(),
    )
...
def apply_fastvid_compression(
    ...
    stats: Dict[str, object] = {
        "method": "fastvid",
        "retention_ratio": float(ext_flags.fastvid_retention_ratio),
        "dyseg_c": int(ext_flags.fastvid_dyseg_c),
        "dyseg_tau": float(ext_flags.fastvid_dyseg_tau),
        "stprune_d": float(ext_flags.fastvid_stprune_d),
        "dtm_p": int(ext_flags.fastvid_dtm_p),
        "dtm_beta": float(ext_flags.fastvid_dtm_beta),
        "score_type": str(ext_flags.fastvid_score_type).strip().lower(),
