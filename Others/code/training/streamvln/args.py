...
class TrainingArguments(transformers.TrainingArguments):
    ...
    lora_include_mm_projector: bool = field(
        default=False,
        metadata={"help": "Whether to include multimodal projector linear layers in LoRA target modules."},
    )
    lora_include_vision_tower: bool = field(
        default=False,
        metadata={"help": "Whether to include vision tower linear layers in LoRA target modules."},
    )
    lora_vision_tower_last_n_layers: int = field(
        default=0,
        metadata={"help": "If >0, only include the last N encoder layers in vision tower for LoRA."},
    )
    lora_disable_bnb: bool = field(
        default=True,
        metadata={"help": "Disable PEFT bitsandbytes dispatch to avoid bnb import/runtime issues on unsupported CUDA setups."},
    )
    lora_bias: str = "none"
