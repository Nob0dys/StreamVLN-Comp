...
def _is_multimodal_module(module_name: str) -> bool:
    multimodal_keywords = [
        "vision_tower",
        "mm_projector",
        "mem_projector",
    ...
    return any(mm_keyword in module_name for mm_keyword in multimodal_keywords)


def _extract_layer_index(module_name: str):
    match = re.search(r"(?:layers|blocks)\.(\d+)", module_name)
    if match is None:
        return None
    return int(match.group(1))


def _get_vision_layer_floor(model, last_n_layers: int):
    if last_n_layers <= 0:
        return None
    ...
    for name, module in model.named_modules():
        if "vision_tower" not in name or not isinstance(module, torch.nn.Linear):
            continue
        layer_id = _extract_layer_index(name)
        if layer_id is not None:
            layer_ids.append(layer_id)

    if len(layer_ids) == 0:
        return None
    return max(layer_ids) - last_n_layers + 1


def find_lora_target_modules(
    model,
    lora_target_modules: str,
    include_mm_projector: bool = False,
    include_vision_tower: bool = False,
    vision_tower_last_n_layers: int = 0,
):
    cls = torch.nn.Linear
    base_targets = [name.strip() for name in lora_target_modules.split(",") if name.strip()]
    if len(base_targets) == 0:
        base_targets = ["q_proj", "v_proj"]
    ...
    vision_layer_floor = _get_vision_layer_floor(model, vision_tower_last_n_layers)
    ...
    for name, module in model.named_modules():
        ...
        is_vision_tower = "vision_tower" in name
        is_mm_projector = (
            "mm_projector" in name or "mem_projector" in name or "point_projector" in name
        )
        is_multimodal = _is_multimodal_module(name)

        if not is_multimodal:
            if leaf_name in base_targets or name in base_targets:
                target_modules.append(name)
                target_stats["llm"] += 1
            continue

        if include_mm_projector and is_mm_projector:
            target_modules.append(name)
            target_stats["projector"] += 1
            continue

        if include_vision_tower and is_vision_tower:
            if vision_layer_floor is not None:
                layer_id = _extract_layer_index(name)
                if layer_id is None or layer_id < vision_layer_floor:
                    continue
            target_modules.append(name)
            target_stats["vision"] += 1

    target_modules = list(dict.fromkeys(target_modules))
    return target_modules, target_stats


def apply_lora_adapters(model, training_args):
    from peft import LoraConfig, get_peft_model

    if training_args.lora_disable_bnb:
        try:
            ...
            rank0_print("Disabled PEFT bitsandbytes dispatch for LoRA injection.")
        except Exception as exc:
            rank0_print(f"Failed to disable PEFT bitsandbytes dispatch cleanly: {exc}")

    lora_target_modules, lora_target_stats = find_lora_target_modules(
        model,
        lora_target_modules=training_args.lora_target_modules,
        include_mm_projector=training_args.lora_include_mm_projector,
        include_vision_tower=training_args.lora_include_vision_tower,
        vision_tower_last_n_layers=training_args.lora_vision_tower_last_n_layers,
    )
    if len(lora_target_modules) == 0:
        raise ValueError(
            "No LoRA target modules were found. Please check lora_target_modules and multimodal LoRA flags."
        )

    rank0_print(
        "LoRA target module stats: "
        f"llm={lora_target_stats['llm']}, "
        f"projector={lora_target_stats['projector']}, "
        f"vision={lora_target_stats['vision']}, "
        f"total={len(lora_target_modules)}"
    )
    if len(lora_target_modules) <= 50:
        rank0_print(f"LoRA target modules: {lora_target_modules}")
    else:
        rank0_print(f"LoRA target modules (first 50): {lora_target_modules[:50]}")
...
def train(attn_implementation=None):
    ...
    pending_lora_application = training_args.lora_enable
    ...
    if model_args.vision_tower is not None:
        ...
        if pending_lora_application:
            model = apply_lora_adapters(model, training_args)
            pending_lora_application = False
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if training_args.lora_enable and "mm_lora_layer" not in tunable_parts:
                rank0_print("LoRA is enabled; auto-appending 'mm_lora_layer' to mm_tunable_parts.")
                tunable_parts.append("mm_lora_layer")
